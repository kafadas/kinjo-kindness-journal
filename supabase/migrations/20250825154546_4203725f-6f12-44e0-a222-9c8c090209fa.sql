-- Create generate_reflection_period RPC function
...
BEGIN
  ...
  SELECT 
    COUNT(*)::integer,
    COUNT(*) FILTER (WHERE action = 'given')::integer,
    COUNT(*) FILTER (WHERE action = 'received')::integer,
    COUNT(*) FILTER (WHERE significance = true)::integer
  ...
  IF p_period = '7d' THEN
    ...
    most_active AS (
      SELECT day_name FROM weekday_activity ORDER BY count DESC LIMIT 1
    ),
    quietest_active AS (
      SELECT day_name FROM weekday_activity ORDER BY count ASC LIMIT 1
    )
  ...
  ELSIF p_period = '30d' THEN
    ...
    SELECT jsonb_build_object(
      'week_to_week_change_pct', array_agg(wc.count ORDER BY wc.week_num DESC),
  ...
  ELSIF p_period = '90d' THEN
    ...
    prior_categories AS (
      SELECT c.name, COUNT(*) as count,
             ROUND((COUNT(*)::numeric / GREATEST(COUNT(*) OVER (), 1)) * 100, 1) as share
      FROM public.moments m
      JOIN public.categories c ON c.id = m.category_id
      WHERE m.user_id = p_user
        AND (m.happened_at AT TIME ZONE p_tz)::date BETWEEN (v_start_date - interval '90 days')::date AND (v_start_date - interval '1 day')::date
      GROUP BY c.name
    ),
    top_tags AS (
    ...
    SELECT jsonb_build_object(
      'shifting_categories', '[]'::jsonb, -- Simplified for now
      'longest_gap_closed', null,
      'tags_top', array_agg(tt.tag)
    )
    INTO v_period_specific
    FROM top_tags tt;
  ...
  ELSIF p_period = '365d' THEN
    ...
    WITH first_moment AS (
      SELECT MIN(happened_at)::date as date
      FROM public.moments m
      WHERE m.user_id = p_user
    ),
    best_streak AS (
    ...
    kindest_month AS (
      SELECT month_name FROM monthly_counts ORDER BY count DESC LIMIT 1
    ),
    peak_month AS (
      SELECT month_name FROM monthly_counts ORDER BY count DESC LIMIT 1  
    ),
    ...
    SELECT jsonb_build_object(
      ...
      'top_connections', array_agg(jsonb_build_object('display_name', tc.display_name, 'shared_count', tc.shared_count))
    )
    INTO v_period_specific
    FROM first_moment fm, best_streak bs, top_connections tc
...
-- Create get_reflection_period RPC function
CREATE OR REPLACE FUNCTION public.get_reflection_period(
  p_user uuid, 
  p_period text, 
  p_tz text DEFAULT 'UTC'
)
...
DECLARE
  v_start_date date;
  v_end_date date;
  v_cached_reflection record;
  v_cache_valid boolean := false;
BEGIN
  -- Calculate expected date range for this period
  v_end_date := (current_timestamp AT TIME ZONE p_tz)::date;
  CASE p_period
    WHEN '7d' THEN v_start_date := v_end_date - interval '6 days';
    WHEN '30d' THEN v_start_date := v_end_date - interval '29 days';
    WHEN '90d' THEN v_start_date := v_end_date - interval '89 days';
    WHEN '365d' THEN v_start_date := v_end_date - interval '364 days';
    ELSE RAISE EXCEPTION 'Invalid period. Must be one of: 7d, 30d, 90d, 365d';
  END CASE;

  -- Check for cached reflection
  SELECT r.* INTO v_cached_reflection
  FROM public.reflections r
  WHERE r.user_id = p_user 
    AND r.period = p_period
    AND r.range_start = v_start_date
    AND r.range_end = v_end_date
  ORDER BY r.created_at DESC
  LIMIT 1;

  -- Determine if cache is valid (exists and within the current period window)
  IF FOUND THEN
    v_cache_valid := true;
  END IF;

  -- Return cached result or generate new one
  IF v_cache_valid THEN
    RETURN QUERY
    SELECT v_cached_reflection.id, v_cached_reflection.user_id, v_cached_reflection.period,
           v_cached_reflection.range_start, v_cached_reflection.range_end,
           v_cached_reflection.summary, v_cached_reflection.suggestions,
           v_cached_reflection.computed, v_cached_reflection.model,
           v_cached_reflection.created_at, v_cached_reflection.regenerated_at;
  ELSE
    -- Generate new reflection
    RETURN QUERY
    SELECT * FROM public.generate_reflection_period(p_user, p_period, p_tz);
  END IF;
END;
$$;