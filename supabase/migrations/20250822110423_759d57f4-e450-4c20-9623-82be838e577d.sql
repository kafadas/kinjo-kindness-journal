-- =========================
-- Phase 2: Kinjo schema (Fixed RLS)
-- =========================
...
-- 5) PEOPLE ↔ GROUPS (junction table)
...
drop policy if exists "ro_user_rows_profiles" on profiles;
drop policy if exists "rw_user_rows_profiles" on profiles;

-- Profiles policies
create policy "ro_user_rows_profiles"
  on profiles for select using (user_id = auth.uid());
create policy "rw_user_rows_profiles"
  on profiles for all using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Create policies for tables with direct user_id
do $$
declare t text;
begin
  for t in
    select unnest( array[
      'categories','groups','people','moments',
      'attachments','streaks','settings','reflections','nudges'
    ] )
  loop
    execute format('drop policy if exists "ro_user_rows_%1$s" on %1$s', t);
    execute format('drop policy if exists "rw_user_rows_%1$s" on %1$s', t);
    execute format('create policy "ro_user_rows_%1$s" on %1$s for select using (user_id = auth.uid())', t);
    execute format('create policy "rw_user_rows_%1$s" on %1$s for all using (user_id = auth.uid()) with check (user_id = auth.uid())', t);
  end loop;
end$$;

-- Junction table policies (person_groups) - access via referenced tables
drop policy if exists "ro_user_rows_person_groups" on person_groups;
drop policy if exists "rw_user_rows_person_groups" on person_groups;

create policy "ro_user_rows_person_groups"
  on person_groups for select using (
    exists (
      select 1 from people p where p.id = person_id and p.user_id = auth.uid()
    )
  );

create policy "rw_user_rows_person_groups"
  on person_groups for all using (
    exists (
      select 1 from people p where p.id = person_id and p.user_id = auth.uid()
    )
  ) with check (
...
drop policy if exists "prompts_read_global_or_own" on prompts;
drop policy if exists "prompts_rw_own" on prompts;
create policy "prompts_read_global_or_own"
on prompts for select using (user_id is null or user_id = auth.uid());
create policy "prompts_rw_own"
...
-- Storage policies
drop policy if exists "storage_read_own_attachments" on storage.objects;
drop policy if exists "storage_write_own_attachments" on storage.objects;
drop policy if exists "storage_delete_own_attachments" on storage.objects;

create policy "storage_read_own_attachments"
on storage.objects for select
using (bucket_id = 'attachments' and ( (storage.foldername(name))[1]::uuid = auth.uid() ));

create policy "storage_write_own_attachments"
on storage.objects for insert
with check (bucket_id = 'attachments' and ( (storage.foldername(name))[1]::uuid = auth.uid() ));

create policy "storage_delete_own_attachments"
