...
-- Drop existing _v1 functions first (if they exist)
DROP FUNCTION IF EXISTS public.daily_moment_counts_v1(uuid,date,date,text,boolean,text);
DROP FUNCTION IF EXISTS public.category_share_delta_v1(uuid,date,date,text,boolean,text);

-- DAILY COUNTS (timezone-safe)
create or replace function public.daily_moment_counts_v1(
  p_user uuid,
  p_start date,            -- null = use earliest moment (in tz)
  p_end   date,            -- null = today (in tz)
  p_action text,           -- 'given' | 'received' | 'both'
  p_significant_only boolean,
  p_tz    text             -- e.g. 'UTC', 'Europe/London'
)
returns table(
  d date,
  given_count int,
...
total as (
  select greatest(sum(cnt), 1) as t from filtered  -- avoid div by zero
)
select
  c.id as category_id,
  c.name,
...
-- Helpful execute grants
grant execute on function public.daily_moment_counts_v1(uuid,date,date,text,boolean,text) to anon, authenticated;
grant execute on function public.category_share_delta_v1(uuid,date,date,text,boolean,text) to anon, authenticated;